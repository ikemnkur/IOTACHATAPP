...
import {
  LogIn, Eye, EyeOff, ShieldCheck, Mail, KeyRound, QrCode, Copy, Check,
  Loader2, RefreshCcw, CheckCircle2, AlertCircle,
} from 'lucide-react';
import { useAuth } from '../context/AuthContext';
import { api, ApiError } from '../lib/api';

type Step = 'credentials' | 'email_verification' | 'totp' | 'setup_qr' | 'setup_confirm' | 'recovery_codes';
...
export default function Login() {
  ...
  const { login, twoFAChallenge, setup2FA, enable2FA, verifyTOTP, useRecoveryCode, isAuthenticated, googleAuth } = useAuth();
  ...
  // ── 2FA setup (only if server says requires2FASetup) ──
  ...
  useEffect(() => {
    ...
    if (twoFAChallenge.type === 'needs_setup') {
      setStep('setup_qr');
    }
  }, [twoFAChallenge, step]);
  ...
  // ── Step: 2FA setup (required for first login) ──
  if (step === 'setup_qr') {
    return (
      <div className="max-w-sm mx-auto py-12">
        <div className="text-center mb-8">
        ...
        <div className="bg-surface-2 rounded-2xl p-6 space-y-4 text-center">
          ...
          <p className="text-sm text-text-muted">
            Download <strong className="text-text">Google Authenticator</strong> or <strong className="text-text">Authy</strong>, then tap below to generate your QR code.
          </p>
      ...
      <div className="max-w-sm mx-auto py-12">
        ...
        <div className="bg-surface-2 rounded-2xl p-6 space-y-4">
          ...
          <form onSubmit={handleEnable} className="space-y-3">
            <div>
              <label className="block text-xs text-text-muted mb-1">Confirm with 6-digit code</label>
    ...
    return (
      <div className="max-w-sm mx-auto py-12">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-text-muted">Store these safely. Each code can only be used once.</p>
        </div>
        <div className="bg-surface-2 rounded-2xl p-6 space-y-4">
          {error && (
            <div className="bg-danger/10 border border-danger/30 text-danger text-sm rounded-lg px-3 py-2">{error}</div>
          ...
          <div className="bg-surface-3 rounded-lg p-3 space-y-1">
            {recoveryCodes.map((code) => (
              <p key={code} className="font-mono text-sm text-text tracking-wider">{code}</p>
            ))}
          </div>
          ...
          <p className="text-xs text-text-muted text-center">2FA is active. You will be redirected after this step.</p>
        </div>
      </div>
    );
  }
